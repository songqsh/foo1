...
# convert a sudoku table of 4 by 4 array into 64 by 1 binary array
...
# convert a 64 by 1 sudoku solution to 4 by 4 table
    ...
    for i_row in range(sudoku_dim):
        ...
        for i_col in range(sudoku_dim):
            ...
            for i_num in range(sudoku_dim):
                if sudoku_solution[i_solution] > 0.9:
                    current_num = i_num + 1
                i_solution += 1
...
def box_condition(i_box, i_num):
    row = [0.] * n_decision_variables  # initialize a row
    i_start_row = int(i_box / 2) * box_row_n
    i_start_col = int(i_box % 2) * box_col_n
    for element in range(sudoku_dim):
        ind1 = conversion_3d_1d(i_start_row + int(element / 2), i_start_col + int(element % 2), i_num)
        row[ind1] = 1.
    return row
